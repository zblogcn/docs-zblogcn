...
**重要：对于第 1 种方法，可能需要额外对 web 空间进行设置，参见「[常见问题](/php/api-faq#apache-获取不到-authorization-头信息 "常见问题")」；**
...
不同「API 模块」专有的查询参数见：「[接口列表](/php/api-mods "接口列表")」；
...
> 原因：未登录或登录用户权限不够；「[登录和鉴权](/php/api-design#权限认证 "登录和鉴权")」
...
> 原因：未正确设置鉴权 Token；「[登录和鉴权](/php/api-design#权限认证 "登录和鉴权")」
