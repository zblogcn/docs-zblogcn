...
1. **开发模式下不报错很重要：「[启用开发模式](/php/dev-start#开发模式 "启用开发者模式")」；**
2. **对于需要用户点链接或提交表单触发，进而对数据或文件产生影响的，除必要的用户权限验证外，应另外加入 CSRF Token 验证；「[CSRF Token 验证](/php/dev-plugin#csrf-相关重要 "CSRF Token 验证")」**
...
13. 在「公共头部」「公共尾部」模板内应各自放置`{$header}`或`{$footer}`标签；「[示例 - 主入口模板 - 主题开发](/php/dev-theme#示例 "示例 - 主入口模板 - 主题开发")」
