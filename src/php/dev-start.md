...
- 按照既定规范组织文件和内容；
- 以统一的方式「安装/删除」「启用/关闭」；
- 可按照功能目的使用接口，也可以不用；「[接口列表](/php/dev-interfaces "docs-zblogphp 接口列表")」
- 「自述文件」为 `plugin.xml` ；
...
- 为避免重复及便于识别，可以增加标识前缀，使用 `_` 与主体分隔；（注2）
- 对于「应用目录」下的全部子目录及文件，原则上建议全部小写；

**注 1：「应用目录」及「自述文件」可按下方操作自动生成；**

**注 2：** 应用 ID 在应用中心全局唯一，直接用功能名（如 `botblock`）容易撞车，撞车后无法上架。建议用「品牌/作者标识 + `_` + 功能名」，如 `qing_botblock`、`wdssmq_links`。
