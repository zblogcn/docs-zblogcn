...
|          |            | `act=list`方法共通参数见：[约束与过滤](/php/api-design#约束与过滤 "约束与过滤")
...
```
此处`login`操作返回的「鉴权 Token」将用于后续需要「鉴权」的请求，见「[权限认证](/php/api-design#权限认证 "权限认证")」；
</details>
...
`"Level": "4",` 为用户等级；参考「[用户等级划定](/php/start-faq#用户等级划定 "用户等级划定")」
</details>
