...
- 各接口返回信息以实际为准，可参考「[通用返回格式](/php/api-design#通用返回格式 "通用返回格式")」。
- **对于「登录」操作以外的`POST`请求，一律需要设置鉴权；「[登录和鉴权](/php/api-design#权限认证 "登录和鉴权")」**
  - `GET`请求时仅在部分「功能模块」中的部分「只读操作」可以不设置鉴权，此外未鉴权请求频率或查询数量可能受限制；

以下将按「功能模块」进行详细说明。

<!-- @include: ./api-mods/api-mod-member.md -->

<!-- @include: ./api-mods/api-mod-post.md -->

<!-- @include: ./api-mods/api-mod-app.md -->

<!-- @include: ./api-mods/api-mod-category.md -->

<!-- @include: ./api-mods/api-mod-comment.md -->

<!-- @include: ./api-mods/api-mod-module.md -->

<!-- @include: ./api-mods/api-mod-system.md -->

<!-- @include: ./api-mods/api-mod-tag.md -->

<!-- @include: ./api-mods/api-mod-upload.md -->